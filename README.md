...
```
        ╱╲
       ╱ 05 ╲        Axiom (공리)         — 자명한 진리, 불변
      ╱──────╲
     ╱   04   ╲      Doctrine (교리)      — 확립된 표준, 의무
    ╱──────────╲
   ╱     03     ╲    Premise (전제)       — 도메인별 가정, 반증 가능
  ╱──────────────╲
 ╱       02       ╲  Praxis (실천)        — 재사용 도구, 체크리스트
╱──────────────────╲
╲        01        ╱  Postmortem (사후분석) — 사건 기록, 날것의 데이터
 ╲────────────────╱
```

### Level 1: Postmortem (사후분석)
...
| 전환 | 이름 | 조건 | 행동 |
|------|------|------|------|
| 01→02 | **도구화** | 동일 유형 사건 2회+, 또는 재발 방지 도구 제작 가능 | 스크립트·체크리스트로 추출 |
| 02→03 | **일반화** | 2개+ 서로 다른 프로젝트/도메인에서 사용됨 | 구체 도구 → 추상 설계 원칙 추출 |
| 03→04 | **표준화** | 3개+ 프로젝트에서 반례 없이 유효 | 의무 적용 표준으로 격상 |
| 04→05 | **철학화** | 기술 스택이 바뀌어도 본질 불변, 1문장 압축 가능 | 기술 구체사항 제거, 보편 원리만 남김 |
...
id: AX-001
level: axiom
promoted_from: 04_doctrine/data_before_code.md
...
- `build_claude_md.sh`가 `05_axiom/` + `04_doctrine/` 내용을 합쳐서 CLAUDE.md 생성
...
- `02_praxis/`: 디버깅 도구/체크리스트
- `01_postmortem/`: 유사 사건 기록
...
2. 작업 중 — 어려운 버그 해결
   └→ ~/episteme/01_postmortem/에 기록
   └→ ~/episteme/sync.sh 실행

3. 재발 방지 도구 제작 가능?
   └→ ~/episteme/02_praxis/에 스크립트/체크리스트 작성
   └→ ~/episteme/sync.sh 실행
...
├── README.md              ← 이 문서
├── build_claude_md.sh     ← 05_axiom + 04_doctrine → CLAUDE.md 빌드
├── sync.sh                ← 충돌 안전 git 동기화
├── 05_axiom/              ← Level 5: 공리
├── 04_doctrine/           ← Level 4: 교리
├── 03_premise/            ← Level 3: 전제
├── 02_praxis/             ← Level 2: 실천
├── 01_postmortem/         ← Level 1: 사후분석
