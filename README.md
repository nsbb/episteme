...
### 계층별 기호 후보 (논리학/수학 유니코드)

> 기호는 미확정. 아래 후보 중 추후 선택 예정.

**01 Axiom (공리) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| ⊤ | U+22A4 | 논리학 Verum | "항상 참" — 공리는 증명 없이 참 |
| ⊢ | U+22A2 | Turnstile | `⊢P` = "P는 공리/정리". 증명의 출발점 |
| ∀ | U+2200 | 전칭 한정사 | "모든 ~에 대해" — 공리의 보편성 |
| Γ | U+0393 | 감마 | 형식논리에서 공리/전제 집합을 표기 |

**02 Doctrine (교리) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| □ | U+25A1 | 양상논리 필연 | "반드시 참" — 의무 적용 표준 |
| ⊨ | U+22A8 | Double turnstile | "의미론적으로 타당" — 검증된 규범 |
| ∴ | U+2234 | 수학 증명 | "고로" — 검증된 결론 |

**03 Premise (전제) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| ∵ | U+2235 | 수학 증명 | "왜냐하면" — 전제를 진술할 때 사용 |
| ◇ | U+25C7 | 양상논리 가능 | "참일 수 있다" — 반증 가능한 가설 |
| ⊃ | U+2283 | 실질 함축 | "~이면 ~이다" — 조건부 가정 |

**04 Praxis (실천) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| λ | U+03BB | 람다 대수 | 함수/계산 — 실행 가능한 코드 |
| → | U+2192 | 함축/사상 | 입력→출력 — 행동과 변환 |
| ⊸ | U+22B8 | 선형 논리 | 자원을 소비해 결과를 생산 |

**05 Postmortem (사후분석) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| † | U+2020 | 학술 표기 | Dagger — 문자 그대로 "죽음" 표시 |
| ∃ | U+2203 | 존재 한정사 | "~인 경우가 존재한다" — 구체적 사건 |
| ⊥ | U+22A5 | Falsum | 모순/실패 — 뭔가 깨졌다 |

**Episteme (전체) — 후보:**

| 기호 | 유니코드 | 출처 | 의미 |
|------|----------|------|------|
| ∴ | U+2234 | 수학 증명 | "고로" — 지식의 최종 결론 |
| ∀ | U+2200 | 전칭 한정사 | "모든 것에 대해" — 보편적 지식 |
| Γ | U+0393 | 감마 | 지식 집합 전체 |
